 ...
 */
describe('List Wineries Dropdown Integration Test', function() {
  ...
  it('should load analysis types data directly from the API with pagination',
      async () => {
        const results =
            await appTester(
                App.triggers.listWineriesDropdown.operation.perform, bundle,
            );
        ...
        results.forEach((winery) => {
          winery.should.have.property('id').which.is.a.String();
          winery.should.have.property('name').which.is.a.String();
        });
        ...
        should(results.length).be.aboveOrEqual(0);
      });
});
